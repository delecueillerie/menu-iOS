...
- (NSDate *)mostRecentUpdatedAtDateForEntityWithName:(NSString *)entityName {
    ...
    [[[menuCoreDataController sharedInstance] backgroundManagedObjectContext] performBlockAndWait:^{
        NSError *error = nil;
        NSArray *results = [[[menuCoreDataController sharedInstance] backgroundManagedObjectContext] executeFetchRequest:request error:&error];
...
- (void)newManagedObjectWithClassName:(NSString *)className forRecord:(NSDictionary *)record {
    NSManagedObject *newManagedObject = [NSEntityDescription insertNewObjectForEntityForName:className inManagedObjectContext:[[menuCoreDataController sharedInstance] backgroundManagedObjectContext]];
...
- (NSArray *)managedObjectsForClass:(NSString *)className withSyncStatus:(SDObjectSyncStatus)syncStatus {
    ...
    NSManagedObjectContext *managedObjectContext = [[menuCoreDataController sharedInstance] backgroundManagedObjectContext];
...
- (void)downloadDataForRegisteredObjects:(BOOL)useUpdatedAtDate {
    ...
    //  NSMutableURLRequest *request = [[SDAFParseAPIClient sharedClient] GETRequestForAllRecordsOfClass:className updatedAfterDate:[NSDate dateWithTimeIntervalSince1970:100]];
        ...
        AFHTTPRequestOperation *operation = [[SDAFParseAPIClient sharedClient] HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObject) {
            if ([responseObject isKindOfClass:[NSDictionary class]]) {
                ...
                [self writeJSONResponse:responseObject toDiskForClassWithName:className];                // Need to write JSON files to disk
...
- (void)executeSyncCompletedOperations {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSError *error = nil;
        [[menuCoreDataController sharedInstance] saveBackgroundContext];
        if (error) {
            NSLog(@"Error saving background context after creating objects on server: %@", error);
        }
        
        [[menuCoreDataController sharedInstance] saveMasterContext];
...
//You won’t really need the JSON responses that were saved to disk much past this point, so add another method to delete them when you’re finished with them
